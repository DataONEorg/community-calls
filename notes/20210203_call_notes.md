...
* Kerstin Lehnert - CZO is a new effort within a new program within NSF. Network of 9 clusters that have different scientific foci. Each cluster is a mini network. KL involved in building data infrastructure for CZ with Hydroshare - submission and access for PIs. has already approach DataONE due to DataONE hosting a lot of data the CZ would like to integrate with. No need to rebuild somehting that exists. The other network is samples - physical onjects that many studies focus on that generate data. Internet of samples and RCN just funded (but not started). Connection btn DataONE and sample networks provides new oppotrunities. Shared infrastructure - Council of Data Facilities. CDF, formed through Earth Cube, forum for discussion. has had a 2 yr WG exploring shared infrastructure. Also, 1Geochemisty: International group trying to build data standards for lab analytical data. dataONE network would bring a lot of structure to a new effort trying to build data standards, best practices etc. 
